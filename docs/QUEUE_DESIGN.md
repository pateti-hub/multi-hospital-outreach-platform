...
The persistent scheduler uses one transaction to:

1. acquire a tenant-specific PostgreSQL advisory lock;
...
The current prototype constrains retries by availability time and clinical cutoff.
Calling-hour, holiday and exact natural-language callback interpretation are documented
as deliberate follow-on work. Requested callbacks use `CALLBACK_SCHEDULED`.
...
Campaign pause stops new reservations but does not terminate active calls. Resume makes
eligible work reservable again. Full dynamic reprioritization is a documented tradeoff.